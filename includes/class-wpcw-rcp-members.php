	...
	class WPCW_RCP_Members
	{
		...
		function attachToTools() 
		{
			...
			add_filter('wpcw_extensions_access_control_override', array($this, 'filter_accessControlForUsers'));
				...
				add_filter( 'rcp_metabox_excluded_post_types', array( $this, 'wpcw_rcp_exclude_post_types' ) );
		...
		function showMembershipMappingLevels()
		{
			...
			$page->showPageHeader($this->extensionName . ' ' . __('&amp; Automatic Course Access Settings', 'wpcw-rcp-addon'), false, WPCW_icon_getPageIconURL());
			...
			if (isset($_GET['level_id']))
			{
				...
				if ($levelID) 
				{
					...
					// Not found in list, show an error.
						...
						$page->showMessage(__('That membership does not appear to exist.', 'wpcw-rcp-addon'), true);
					}
		...
		private function showMembershipMappingLevels_specificLevel($page, $levelDetails)
		{
			...
			printf('<div id="wpcw_member_level_name_title">%s</div>', 
				sprintf(__('Editing permissions for <b>%s</b> membership with <b>%s</b>:', 'wpcw-rcp-addon'), $levelDetails['name'], $this->extensionName)
			);
			...
			$form->setSubmitLabel(__('Save Changes', 'wpcw-rcp-addon'));
			
			// Create list of courses using checkboxes (max of 2 columns)
			$elem = new FormElement('level_courses', __('Courses user can access with this membership', 'wpcw-rcp-addon'), false);
			$elem->setTypeAsCheckboxList($courses);
			$elem->checkboxListCols = 2;
			$form->addFormElement($elem);

			// Create retroactive option
			$elem = new FormElement('retroactive_assignment', __('Do you want to retroactively enroll current members into these courses?', 'wpcw-rcp-addon'), true);
			$elem->setTypeAsRadioButtons(array(
		'Yes'				=> __('Yes', 'wpcw-rcp-addon'),
		'No'				=> __('No', 'wpcw-rcp-addon'),	
		));
			$form->addFormElement($elem);
			...
			if ($form->formSubmitted())
			{
				if ($form->formValid())
				{
					...
					if ($retroactive_assignment == 'Yes' && count($mapplingList) >= 0){
						$level_ID = $levelDetails['id'];
						$this->retroactive_assignment($level_ID);
						//$page->showMessage(__('All members were successfully retroactively enrolled into the selected courses.', 'wpcw-rcp-addon'));
					}					
					
					// Show a success message.
					$page->showMessage(
						__('Level and course permissions successfully updated.', 'wpcw-rcp-addon') . '<br/><br/>' .
						sprintf(__('Want to return to the <a href="%s">Course Access Settings summary</a>?', 'wpcw-rcp-addon'), $summaryURL) 
					);
			...
			printf('<a href="%s" class="button-secondary">%s</a>', $summaryURL, __('&laquo; Return to Course Access Settings summary', 'wpcw-rcp-addon'));
		...
		private function showMembershipMappingLevels_overview($page)
		{
			...
			if ($levelData)
			{
				...
				$col = new TableColumn(__('Membership ID', 'wpcw-rcp-addon'), 'wpcw_members_id');
				$table->addColumn($col);
				
				$col = new TableColumn(__('Membership Name', 'wpcw-rcp-addon'), 'wpcw_members_name');
				$table->addColumn($col);
				
				$col = new TableColumn(__('Users with this membership can access:', 'wpcw-rcp-addon'), 'wpcw_members_levels');
				$table->addColumn($col);
				
				$col = new TableColumn(__('Actions', 'wpcw-rcp-addon'), 'wpcw_members_actions');
				$table->addColumn($col);
				...
				foreach ($levelData as $id => $levelDatum)
				{
					...
					// No courses yet
						...
						$data['wpcw_members_levels'] = __('There are no courses yet.', 'wpcw-rcp-addon');	
					}
					...
					$data['wpcw_members_actions'] 	= sprintf('<a href="%s&level_id=%s" class="button-secondary">%s</a>', 
						$baseURL, $levelDatum['id'], __('Edit Course Access Settings', 'wpcw-rcp-addon')
					);					
			...
			// Nothing found, show nice error message.
				...
				$page->showMessage(sprintf(__('No memberships were found for %s.', 'wpcw-rcp-addon'), $this->extensionName), true);
			}			
		}
		...
		{
			printf('<div class="error"><p><b>%s - %s %s:</b> %s</p></div>', 'WP Courseware', $this->extensionName, __('addon', 'wpcw-rcp-addon'), 
				sprintf(__('The %s plugin has not been detected. Is it installed and activated?', 'wpcw-rcp-addon'), $this->extensionName)
			);
		}
		...
		{
			printf('<div class="error"><p><b>%s</b> %s</p></div>', __('WP Courseware', 'wpcw-rcp-addon'), __('has not been detected. Is it installed and activated?', 'wpcw-rcp-addon'));
		}
		...
		public function filter_accessControlForUsers($existing)
		{
			...
			return $existing.sprintf('<li class="wpcw_bullet wpcw_override">%s <a href="%s">%s</a> %s</li>', 
				__('New users given access based on', 'wpcw-rcp-addon'), 
				admin_url('admin.php?page=' . $this->extensionID),
				$this->extensionName,
				__('membership', 'wpcw-rcp-addon'));
		}
